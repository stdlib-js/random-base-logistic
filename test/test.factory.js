...
tape( 'attached to the returned function is the generator name', function test( t ) {
	...
	t.strictEqual( logistic.NAME, 'logistic', 'has property' );
	t.end();
});

tape( 'attached to the returned function is the underlying PRNG', function test( t ) {
	var logistic = factory();
	t.strictEqual( typeof logistic.PRNG, 'function', 'has property' );

	logistic = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( logistic.PRNG, minstd.normalized, 'has property' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed (integer seed)', function test( t ) {
	...
	t.strictEqual( isUint32Array( logistic.seed ), true, 'has property' );
	t.strictEqual( logistic.seed[ 0 ], 12345, 'equal to provided seed' );

	logistic = factory({
		'seed': 12345,
		'prng': minstd.normalized
	});
	t.strictEqual( logistic.seed, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed length', function test( t ) {
	var logistic = factory();
	t.strictEqual( typeof logistic.seedLength, 'number', 'has property' );

	logistic = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( logistic.seedLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state', function test( t ) {
	var logistic = factory();
	t.strictEqual( isUint32Array( logistic.state ), true, 'has property' );

	logistic = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( logistic.state, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state length', function test( t ) {
	var logistic = factory();
	t.strictEqual( typeof logistic.stateLength, 'number', 'has property' );

	logistic = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( logistic.stateLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state size', function test( t ) {
	var logistic = factory();
	t.strictEqual( typeof logistic.byteLength, 'number', 'has property' );

	logistic = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( logistic.byteLength, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is a method to serialize the generator as a JSON object', function test( t ) {
	...
	logistic = factory();
	t.strictEqual( typeof logistic.toJSON, 'function', 'has method' );

	o = logistic.toJSON();
	t.strictEqual( o.type, 'PRNG', 'has property' );
	t.strictEqual( o.name, logistic.NAME, 'has property' );
	t.deepEqual( o.state, typedarray2json( logistic.state ), 'has property' );
	t.deepEqual( o.params, [], 'has property' );

	logistic = factory( 1.0, 2.0 );
	o = logistic.toJSON();
	...
	t.strictEqual( typeof logistic.toJSON, 'function', 'has method' );
	t.strictEqual( logistic.toJSON(), null, 'returns expected value' );
...
tape( 'the function supports providing a seeded underlying PRNG (parameters)', function test( t ) {
	...
	for ( i = 0; i < 1e2; i++ ) {
		r1 = logistic1( 1.0, 2.0 );
		r2 = logistic2( 1.0, 2.0 );
		t.strictEqual( r1, r2, 'both return same number' );
	}
	t.end();
});
...
tape( 'the function supports specifying the generator state', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		t.strictEqual( logistic(), arr[ i ], 'returns expected value. i: '+i+'.' );
	}
	t.end();
});
